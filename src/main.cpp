...
void applyState(AlertState state) {
    switch (state) {
        ...
        case UNSAFE:
            blinkOn = true;
            setColor(80, 0, 0);       // Red (blink handled in loop)
            showScroll("UNSAFE");
            break;
    }
}
        ...
        if (payload.length() > 10) {
            ...
            if (!err) {
                ...
                for (JsonVariant v : data) {
                    ...
                    // Match partial: handles "רמת השרון - כל האזורים" variants too
...
void loop() {
    // Drive matrix animation (scrolling)
    if (matrix.displayAnimate() && currentState != SAFE) {
        matrix.displayReset();   // Loop scroll
    }

    // Blink NeoPixel red when UNSAFE
    if (currentState == UNSAFE && millis() - lastBlink >= 500) {
        lastBlink = millis();
        blinkOn = !blinkOn;
